...
self.port.on("getImageUrls", function(imageUrls){
    console.log("setting image urls");
    iconUrl = imageUrls["iconUrl"];
    pointsDoneUrl = imageUrls["pointsDoneUrl"];
    ready = true;
});
...
var onDocLoad = function(){
	...
	$('.js-toggle-label-filter, .js-select-member, .js-due-filter, .js-clear-all').on('mouseup', updateFilters);
	$('.js-input').on('keyup', updateFilters);


	//for storypoint picker
	$(".card-detail-title .edit-controls").on('DOMNodeInserted',showPointPicker);
